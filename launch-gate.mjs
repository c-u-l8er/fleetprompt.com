/* ==========================================================================
   fleetprompt.com publication gate. No dependencies.

       node launch-gate.mjs        (run it via ./site.sh)

   It reads the ARTIFACT — the emitted index.html, replay.js and form.js — and
...
const ARTIFACTS = ["./index.html", "./replay.js", "./form.js"];
   ...
   form's progressive upgrade. */
{
    ...
    T("the landing page loads only the nav, the animation and the form upgrade",
        srcs.length === 3 && srcs.includes("/amp-nav.js") && srcs.includes("/replay.js") && srcs.includes("/form.js"),
        srcs.join(", "));
    T("the animation and the form upgrade are deferred",
        tags.filter((t) => /\/(replay|form)\.js/.test(t[1])).every((t) => /\bdefer\b/.test(t[1])));
}
...
T("/ carries the retraction rather than a silent edit",
    /Retraction &mdash;/.test(landing) && /No gate would have caught this/.test(landing));
...
T("the placement band bounds what its rung covers", landing.includes(surface.surface_rung_covers));
{
    ...
    T("/ embeds the nav whose placement it agrees with",
        landing.includes(`<amp-nav property="fleetprompt">`));
}
...
/* ---------- 12. the published counts are the ones that were run ---------- */
{
    ...
    /* NOT "every suite is green". The tagged-out run FAILS, and publishing that
       is the point: a green count that depends on two tests not running is half
       a fact. The check is that the two runs account for each other. */
   ...
   allowed to move it; it is not allowed to soften it. */
{
    /* THE VOCABULARY IS HARDCODED HERE, NOT READ FROM THE RECORD, and that is
       ...
       break found it on this surface AND on its sibling. A check whose
       expectation is edited by the same commit as the thing it checks is not a
       check; the gate is the law and the record is the data. VISIBLE covers
       the install-path tags as well as the pieces table, which is how the
       second occurrence of this hole — a step tag reading "gate coming soon"
       — is caught by the same three lines. */
    ...
    /* The three install-path tags are the same vocabulary in a different
       widget, and a redesign that softens them softens the page. */
    const STEP_TAGS = new Set(["gate not built", "cli not built", "substrate ships"]);
    const oddTags = surface.steps.map((x) => x.tag).filter((t) => !STEP_TAGS.has(t));
    ...
    /* ---------- THE SENTENCE THIS DOMAIN IS JUDGED ON ----------
       ...
       should copy, and this row is the reason. It is also the row a redesign
       is most tempted to lose, because it is the one that says the product has
       no customers. Checked as three literal fragments in one row rather than
    ...
    /* And the three fragments must be in the SAME row, not scattered across
       the page in a way that reads as a claim rather than as an admission. */
    const rowHtml = (/<tr>(?:(?!<\/tr>)[\s\S])*Second publisher[\s\S]*?<\/tr>/.exec(landing) || [""])[0];
    T("the three fragments are one row, not three sentences",
        ROW.every((f) => rowHtml.includes(f)), rowHtml ? "one <tr>" : "no row found");

    /* SITES.md §0.7 names this exact defect by name: a proposed CTA of
       "become publisher #2", pointed at a registry whose publish CLI and gate
       are both `not built` and which says of itself that there is nowhere in
       it to put a film. A page that cannot honour its own call to action is
       worse than a page with none, because the visitor finds out afterwards. */
...

{
    ...
    T("no text on the landing page is a constant read from the animation", leaked.length === 0,
        leaked.length
            ? `LEAKED: ${leaked.map((l) => JSON.stringify(l)).join(", ")} — change src/replay.js, not the page`
...
const CONTRAST_PAIRS = [
    ...
    ["--fg", "--ink3", "the registry entry's header, table headers' raised surface"],
    ...
    ["--acc", "--ink2", "CTA verbs, eyebrows, the logo on hover, the step numbers"],
    ["--acc", ["--ink2", "--acc-soft"], "a CTA card while hovered"],
    ["--data", "--ink2", "the registry entry's identity, the where-tag, measured statuses"],
